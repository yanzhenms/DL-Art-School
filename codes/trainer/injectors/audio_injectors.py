...
class OptimTransPairInjector(Injector):
    ...
    def forward(self,state):
        with torch.no_grad():
            data = state[self.input] # data batch
            data_np = data.to('cpu').numpy()
            batch_sz = data_np.shape[0]
            noise_np = np.random.randn(*data_np.shape) # sampling noises 
            M = ot.dist(noise_np.reshape(batch_sz,-1),data_np.reshape(batch_sz,-1),metric='euclidean') # calculating euclidean distance with data batch
            M = pow(M,2) # calculating squared distance

            alpha = ot.unif(batch_sz) 
            beta = ot.unif(batch_sz)
            Perm = batch_sz*ot.emd(alpha,beta,M,numItermax=100000) # optimized permutation matrix
            noise_perm = (np.arange(batch_sz)@Perm).astype(np.int32) # noise permutation 
            noise_val_perm = noise_np[noise_perm,:,:] # rearranged noise batch
